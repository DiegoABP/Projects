...
classdef optimizer < handle
  ...
  properties (Access = public)
    ...
    beta1   = 0.70;  ## Momentum: 0 para no usar momentum #0.70
...
#--------------------------CAMBIOS REALIZADOS AL CODIGO----------------------------------"
    #theta update batch
     ...
     ##  Se usa como referencia  tn = tc - alpha * gn;
     ##tomado de batch_grad_descent
      tn = tc - self.alpha*g;
    endfunction

     #theta update sgd
     function tn = updateSgd(self,tc,g)
     ##  Se usa como referencia   tn = tc - alpha * gn;# Next position
     ##tomado de stoch_grad_descent
      tn = tc - self.alpha*g;

    endfunction

  ## ****************** Cambios Tarea 4 ************************************************  
     function tn = updatermsprop(self,tc,g)
     ##  Se usa como referencia   tn = tc - alpha * gn;# Next position
     ##tomado de stoch_grad_descent
      s = g.^2;
     ...
     function tn = updateadam(self,tc,g)
     ##  Se usa como referencia   tn = tc - alpha * gn;# Next position
     ##tomado de stoch_grad_descent
      s = g.^2;
      V = g;
  ...
  methods (Access = public)
    ...
    %
    % Example:
    %   [pos,errors]=optimizer(@loss,@gradloss,[0 0 0.5],X,Y,0.1,"method","adam","maxIter",10)
    function [pos,errors]=minimize(self,J,gJ,theta0,Xo,Yo)
